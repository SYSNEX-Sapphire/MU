                ...
                if (FlowControlConfirmEx.Show("변경 확인", message) == DialogResult.Ok)
                {
                    try
                    {
                        if (controlValues.targetValue != null && controlValues.rampTime != null)
                        {
                            if (ControlMode == PressControlMode.Pressure)
                            {
                                PLCService.WriteGeneralDeviceIOControl(DeviceConfiguration.OutputCmd1Index.PressureControlMode, true);
                            }
                            else
                            {
                                PLCService.WriteGeneralDeviceIOControl(DeviceConfiguration.OutputCmd1Index.PressureControlMode, false);
                            }
